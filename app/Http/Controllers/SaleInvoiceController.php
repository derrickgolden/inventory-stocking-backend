...
class SaleInvoiceController extends Controller
{
    ...
    public function createSingleSaleInvoice(Request $request): JsonResponse
    {
        try {
           ...
           // Ensure counterId is present in the request
        $counterId = $request->input('counterId');
        ...
        $stock = \App\Models\CounterProductStock::where('counter_id', $counterId)
            ->where('product_id', $productId)
            ->first();

        if ($stock && $stock->current_quantity >= $productQuantity) {
            $stock->decrement('current_quantity', $productQuantity);
        } else {
            return response()->json(['error' => 'Insufficient stock at counter'], 400);
        }
                ...
                if ($request->query('user')) {
                    if ($request->query('count')) {
                        ...
                        $aggregation = SaleInvoice::whereBetween('date', [$startDate, $endDate])
                            ...
                            ->when($counterId, function ($query, $counterId) {
                                return $query->where('counterId', $counterId);
                            })
