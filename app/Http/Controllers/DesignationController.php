...
class DesignationController extends Controller
{
    ...
    {
        Log::info("updateSingleDesignation", $request->all(), $id);
        try {
            $data = $request->only(['name']); // ✅ Only update allowed column(s)

            $updatedDesignation = Designation::where('id', $id)->update($data);
            ...
            return response()->json(['message' => 'Designation updated successfully'], 200);
             ...
             Log::error("Failed to update designation ID {$id}", [
