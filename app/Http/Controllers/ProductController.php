    ...
    public function getAllProduct(Request $request): JsonResponse
    {
        ...
        if ($request->query('query') === 'all') {
            ...
            try {
                ...
                // Check if counterId is present
                if ($request->has('counterId')) {
                    $counterId = $request->query('counterId');

                    // Fetch products specific to this counter
                    $counterStock = \App\Models\CounterProductStock::with(['product.productSubCategory', 'product.productBrand', 'product.productColor.color'])
                        ->where('counter_id', $counterId)
                        ->skip($pagination['skip'])
                        ->take($pagination['limit'])
                        ->get();

                    $result = $counterStock->map(function ($stock) {
                        $product = $stock->product;

                        $totalCount = count($product->reviewRating ?? []);

                        $product->totalRating = $totalCount > 0
                            ? $product->reviewRating->reduce(fn($acc, $curr) => $acc + $curr->rating, 0) / $totalCount
                            : 0;

                        $product->productThumbnailImageUrl = $product->productThumbnailImage
                            ? url("/product-image/{$product->productThumbnailImage}")
                            : null;

                        // Optionally add quantity from stock
                        $product->currentQuantity = $stock->current_quantity;
                    ...
                    return response()->json([
                        ...
                        'totalProduct' => \App\Models\CounterProductStock::where('counter_id', $counterId)->count(),
                    ]);
                }

                // Else default to fetching all products
                $getAllProduct = \App\Models\Product::orderBy('id', 'desc')
                ...
                $getAllProduct->map(function ($product) {
                    $totalCount = count($product->reviewRating);
                ...
                return response()->json([
                    'error' => 'An error occurred during getting product. Please try again later.',
                ], 500);
            }
        } else {
            Log::info("getAllProduct", $request->all());
            try {
                $pagination = getPagination($request->query());
                $getAllProduct = Product::orderBy('id', 'desc')
                    ->where('status', 'true')
